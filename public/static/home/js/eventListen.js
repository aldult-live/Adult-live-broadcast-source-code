    ...
    inituser: function () {
        $.ajax({
        	...
        	success:function(data){
        		if(data.error==0){
					socket.emit('conn', {uid:data.userinfo.id,roomnum: data.userinfo.roomnum,nickname: data.userinfo.user_nickname,stream:data.userinfo.stream,equipment: 'pc',token:data.userinfo.token});
	...
	console.log(data);
		if(data[0]=='ok'){
			...
			if(_DATA.user && _DATA.user.id &&  _DATA.anchor.id && _DATA.user.id !=  _DATA.anchor.id){
				...
				var msg = '{"msg":[{"_method_":"requestFans","action":"","timestamp":"'+WlTools.FormatNowDate()+'","msgtype":"1","level":"","uid":"","sex":"","uname":"","uhead":"","usign":"","city":"好像在黑洞","level":""}],"retcode":"000000","retmsg":"ok"}';
