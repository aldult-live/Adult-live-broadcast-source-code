...
class Api_Dynamic extends PhalApi_Api {
	...
	public function getRules() {
		return array(
            'setDynamic' => array(
				...
				'type' => array('name' => 'type', 'type' => 'int','default'=>0, 'desc' => '动态类型：0：纯文字；1：文字+图片；2：文字+视频；3：文字+音频；4：分享商品'),
				'labelid' => array('name' => 'labelid', 'type' => 'int','default'=>0, 'desc' => '标签ID'),
				'goodsid' => array('name' => 'goodsid', 'type' => 'int', 'default'=>0, 'desc' => '分享到动态的商品ID'),
	...
	public function setDynamic() {
       ...
       $issign=checkSign($checkdata,$sign);
		...
		if($type==4){
        	...
        	$goodsinfo=$domain_shop->getGoods($where);
	...
	public function getDynamic() {
        ...
        $dynamicid=checkNull($this->dynamicid);
        $domain = new Domain_Dynamic();
        $result = $domain->getDynamic($uid,$dynamicid);
		if(!$result){
			$rs['code'] = 1000;
			$rs['msg'] = "动态已删除";
			return $rs;
